...
function Method() {
  ...
  return <section className="method-section section" id="method"><div className="container method-grid"><div className="method-copy reveal"><span className="eyebrow light"><i />Подход American Dream</span><h2>Не просто занятия.<br /><em>Путь к результату.</em></h2><p>Мы соединяем понятную программу, живую практику и регулярную обратную связь. Ученик всегда знает, что уже умеет и какой шаг будет следующим.</p><div className="method-photo"><img src="/assets/american-dream-learning-method.png" alt="Преподаватель American Dream занимается со студентами" /></div><Button light>Получить консультацию</Button></div><ol className="steps reveal delay-1">{steps.map(([number, title, text]) => <li key={number}><span>{number}</span><div><strong>{title}</strong><p>{text}</p></div></li>)}</ol></div></section>;
}
...
function Teachers() {
  ...
  return <section className="teachers section" id="teachers"><div className="container teacher-layout"><div className="teacher-visual reveal"><div className="teacher-board" role="img" aria-label="Преподаватель American Dream помогает ученикам"><span>AMERICAN DREAM</span><strong>Сильный учитель<br />создаёт уверенного<br /><em>ученика.</em></strong></div><div className="teacher-chip"><Icon name="chat" /><span><strong>Понятная обратная связь</strong><small>после каждого этапа</small></span></div></div><div className="teacher-copy reveal delay-1"><span className="eyebrow"><i />Команда преподавателей</span><h2>Знания важны.<br /><em>Умение вдохновить — тоже.</em></h2><p>Мы ценим преподавателей, которые умеют удерживать внимание, видеть потребности ученика и превращать урок в понятный путь вперёд.</p><div className="teacher-principles">{principles.map(([title, text], index) => <div key={title}><span>{String(index + 1).padStart(2, '0')}</span><div><strong>{title}</strong><p>{text}</p></div></div>)}</div></div></div></section>;
}
